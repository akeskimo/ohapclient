...
//                }
                    ...
                    try {
                        // put networking thread to sleep
    ...
    } // end of incoming thread inner class
    ...
    public void start(ConnectionObserver observer) {
        // Entry-point for the users of this class to start networking.
    ...
    public void stop() {
        // Should be called before application is closed or networking is stopped
    ...
    private void startNetworking() {
        // will initialize handlers and start up threads

        Log.d(TAG, "startNetworking() Called. Initializing handlers and threads.");
        setRunning(true);
        handlerThread = new HandlerThread("handlerThread");
        handlerThread.start();
        incomingThread = new IncomingThread();
        incomingThread.start();
        Log.d(TAG, "startNetworking() Threads have been started.");
    }

    private void stopNetworking() {
        // Will set handlers to null and stop/join threads that were started in startNetworking()
        // function
    ...
    protected void listeningStateChanged(Container container, boolean listening) {
        // registers new listening containers
        // if amount of listeners is zero, calls start/stop networking

        // NOTE: start/stop listening should be called equal amount or will trigger server error

        if (listening) {
            // if container wants to start listening
            ...
            // container wants to stop listening
    ...
    protected void changeBinaryValue(Device device, boolean value) {
        ...
        sendBinaryValueChanged(device); // notify server
    ...
    protected void changeDecimalValue(Device device, double value) {
        ...
        sendDecimalValueChanged(device); // notify server
